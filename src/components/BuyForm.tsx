...
import { KARAT_PURITIES, calculateBuyPrice } from '../utils/goldCalculations';
...
export const BuyForm: React.FC<BuyFormProps> = ({ currentRate, currency, onSubmit }) => {
  const [weight, setWeight] = useState<number>(0);
  const [karat, setKarat] = useState<number>(24);
  const [tested, setTested] = useState<boolean>(false);
  const [discount, setDiscount] = useState<number>(10); // 10% default discount
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-lg p-6">
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Buy Gold</h2>
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Buy Discount (%)</label>
          <input
            type="number"
            step="0.5"
            min="0"
        ...
        <div className="flex items-center">
          ...
          />
          <label className="ml-2 block text-sm text-gray-700">Gold Tested</label>
        </div>

        <div className="bg-gray-50 p-4 rounded-lg">
          <h3 className="text-lg font-semibold text-gray-700">Estimated Price</h3>
